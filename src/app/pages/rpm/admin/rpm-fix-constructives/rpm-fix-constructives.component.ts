...
import { RpmDeleteConstructive, ListRemark, RpmConstructivesRemark } from '../rpm-constructives/rpm-constructives.component';
import { ToasterComponent } from '../../../../components/toaster/toaster.component';
import { RpmRemarkService } from '../../../../services/rpm/rpm-remark.service';
import { ConstructiveRemarkAdd, UpdateConstructive, DeleteConstructive, RpmConstructivesService } from '../../../../services/rpm/rpm-constructives.service';


declare var jQuery: any;
@Component({
  selector: 'app-rpm-fix-constructives',
  templateUrl: './rpm-fix-constructives.component.html',
  styleUrls: ['./rpm-fix-constructives.component.css']
})
export class RpmFixConstructivesComponent implements OnInit {
  ...
  addConstructive(name: string) {
    this.service.addConstructive(name, 1).subscribe(
      data => {
        if (data.success) {
          this.getConstructives();
  ...
  getConstructives() {
    this.service.getFixConstructives().subscribe(
      data => {
    ...
    this.service.DeleteConstructive(this.delConstructive).subscribe(
      data => {
        ...
        if (data.success) {
          ...
          if (this.editModel.TypeNum === 2) {
            this.loadTypeRemark(this.constructiveRootId, this.delConstructive.typeNum - 1);
          }
          if (this.editModel.TypeNum === 3) {
